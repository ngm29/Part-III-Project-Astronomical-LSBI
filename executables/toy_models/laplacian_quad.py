...
for r in range(rounds):
    ...
    if not r:
        ...
        θ = np.squeeze(posterior.rvs(size=k//N).reshape((k//N)*N, n))
    D = np.array([np.squeeze(Simulator(simulator, m, M, C, ν,Q).rvs(θ[i])) for i in range((k//N)*N)])
