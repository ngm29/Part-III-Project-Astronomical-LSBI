...
for k in k_set:
    ...
    theta_0, D_0, samps, samps2, d_kl = lsbi(d, n, k, N, t=n,  m=m, M=M, C=C,
                                             mu=mu, Sigma=Sigma, simulator='Laplacian', seed=seed)
...
if plot_posterior:
    ...
    plt.savefig('../figures/laplacian.pdf')
    plt.show()
...
if plot_dkl:
    ...
    #plt.title(r'Laplacian likelihood, $n=%s$, $d=%s$'%(n,d))
    plt.savefig('../figures/laplacian_dkl.pdf')
    plt.show()