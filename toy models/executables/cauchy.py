...
from scipy.stats import invwishart, multivariate_t, multivariate_normal, matrix_normal
import numpy as np
...
for k in k_set:
    ...
    theta_0, D_0, samps, samps2, d_kl = lsbi(d, n, k, N, t=n, m=m, M=M, C=C,
                                             mu=mu, Sigma=Sigma, simulator='Student-t', seed=seed)
...
if plot_posterior:
    ...
    #plt.suptitle(r'Student-$t$ likelihood, $n=%s$, $d=%s$'%(n,d))
    plt.savefig('../figures/cauchy.pdf')
    plt.show()
...
if plot_dkl:
    ...
    #plt.title(r'Student-$t$ likelihood, $n=%s$, $d=%s$'%(n,d))
    plt.savefig('../figures/cauchy_dkl.pdf')
    plt.show()
